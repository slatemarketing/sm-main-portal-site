...
import { columns } from "@/components/admin/companies/columns-companies-admin";
...
export default async function AdminCompaniesPage() {
  ...
  const pendingCompanies = data.filter(
    (company) => company.status === "PENDING"
  ).length;
  const inactiveCompanies = data.filter(
    (company) => company.status === "INACTIVE"
  ).length;
  const totalUsersInCompanies = data.reduce(
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage all registered companies and view key metrics
            </p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <CardContent>
                <div className="flex flex-row justify-between">
                  <div className="space-y-1">
                    <h2 className="text-sm font-medium">Total Companies</h2>
            ...
            <Card>
              <CardContent>
                <div className="flex flex-row justify-between">
                  <div className="space-y-1">
                    <h2 className="text-sm font-medium">Active Companies</h2>
                    <h3 className="text-2xl font-bold">{activeCompanies}</h3>
                    <p className="text-sm font-medium text-muted-foreground">
                      {totalCompanies === 0
                        ? "0.0"
